...
async def _get_profile_url() -> str:
    username = os.getenv("MOLTBOOK_USERNAME") or os.getenv("BOT_NAME", AGENT_NAME)
...
async def _do_heartbeat() -> dict:
    ...
    if expected_posts > len(today_posts):
        ...
        # Charakter-Kontext aus Core-Values und Logbuch für authentischere Posts
        _cv_ctx = ""
        try:
            ...
            _cv_ctx = "\n\n".join(filter(None, [_cv, _lb]))
        except Exception:
            pass
        _char_block = (
            f"\n\nDEIN CHARAKTER-KONTEXT (lass das authentisch einfließen, nicht wörtlich zitieren):\n{_cv_ctx}"
            if _cv_ctx else ""
        )
    ...
    own_name = (os.getenv("MOLTBOOK_USERNAME") or os.getenv("BOT_NAME", "RICS")).lower()
    if author_name and author_name not in already_followed and author_name.lower() != own_name:
